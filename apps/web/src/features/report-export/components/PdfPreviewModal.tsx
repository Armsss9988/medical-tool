...
export default function PdfPreviewModal({
  ...
  const safeSelectedTests = useMemo(() => selectedTests || [], [selectedTests]);

  // Nhận diện loại báo cáo qua Domain Service thuần túy
  const reportType = useMemo(
    () => ReportClassificationDomainService.classify(safeSelectedTests),
    [safeSelectedTests]
  );
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="font-extrabold text-sm text-slate-100 flex items-center gap-2">
                ...
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${reportBadge.bgClass} ${reportBadge.textClass} ${reportBadge.borderClass}`}>
                  {reportBadge.label}
                </span>
              </h3>
