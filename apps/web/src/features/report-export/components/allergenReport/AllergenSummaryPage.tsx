...
export interface AllergenSummaryPageProps {
  patient: Patient;
...
function AllergenSummaryPage({
  ...
  const scaleTitle = useMemo(() => {
    if (packageName && packageName.trim() !== '') {
      const cleanPkg = packageName.replace(/^[\p{Emoji}\s]+/u, '').trim();
      if (cleanPkg.startsWith('(') && cleanPkg.endsWith(')')) {
        return `DIỄN GIẢI ĐỘ DƯƠNG TÍNH ${cleanPkg}`;
      }
      return `DIỄN GIẢI ĐỘ DƯƠNG TÍNH (${cleanPkg})`;
    }
    return 'DIỄN GIẢI ĐỘ DƯƠNG TÍNH';
  }, [packageName]);
    ...
    >
      <div>
        ...
        <div className="flex items-start justify-between border-b-2 border-slate-900 pb-2 mb-2">
          <div className="flex items-center space-x-3">
            {currentLogo ? (
              ...
              <div className="h-12 w-12 bg-sky-900 text-white font-black flex items-center justify-center rounded-lg text-base tracking-wider">
                GOLAB
              </div>
            )}
            <div>
              <h1 className="text-[16px] font-black uppercase text-sky-950 tracking-tight leading-none mb-1">
          ...
          <div className="text-right">
            <div className="inline-flex items-center justify-center bg-red-600 text-white font-black text-[12px] px-3 py-1 rounded tracking-wide uppercase leading-normal shadow-xs">
              Báo Cáo Dị Nguyên
            </div>
          </div>
        </div>
        ...
        <div className="text-center mb-2">
          ...
          <p className="text-[12.5px] font-bold text-red-700 italic">
            (Tổng hợp các dị nguyên dương tính & nồng độ IgE toàn phần)
          </p>
        </div>

        {/* Thanh Thông Tin Bệnh Nhân Tóm Tắt */}
        <div className="flex items-center justify-between bg-slate-50 border border-slate-300 rounded px-3.5 py-1 mb-2 text-[12.5px] leading-snug">
          <div>
            <span className="font-semibold text-slate-600">Họ tên: </span>
            <strong className="text-red-600 uppercase font-bold text-[13.5px]">{patient.name || '---'}</strong>
          </div>
          <div>
        ...
        <div className="border border-slate-300 rounded mb-1.5 bg-white">
          <table className="w-full text-[12.5px] border-collapse">
            <thead className="bg-slate-50 text-slate-900 font-bold border-b-2 border-slate-300">
              <tr>
                <th className="py-1.5 px-3 w-12 text-center border-r border-slate-300 align-middle leading-snug">STT</th>
                <th className="py-1.5 px-3 text-left border-r border-slate-300 align-middle leading-snug">LOẠI DỊ NGUYÊN</th>
                <th className="py-1.5 px-3 text-left border-r border-slate-300 align-middle leading-snug">TÊN KHOA HỌC</th>
                <th className="py-1.5 px-3 w-20 text-center border-r border-slate-300 align-middle leading-snug">MÃ</th>
                <th className="py-1.5 px-3 w-32 text-center align-middle leading-snug">ĐỘ DƯƠNG TÍNH</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-300">
              {positiveList.length === 0 ? (
                <tr>
                  <td colSpan={5} className="py-2.5 text-center text-slate-500 italic text-[12.5px]">
                ...
                positiveList.map((pos, idx) => {
                  ...
                  return (
                    <tr key={pos.code || idx} className={`${gradeStyle.rowBg} font-bold ${gradeStyle.textColor} text-[13px]`}>
                      <td className="py-1.5 px-3 text-center border-r border-slate-300 align-middle leading-snug">{idx + 1}</td>
                      <td className={`py-1.5 px-3 border-r border-slate-300 align-middle leading-snug ${gradeStyle.nameColor}`}>{pos.name}</td>
                      <td className="py-1.5 px-3 border-r border-slate-300 italic font-medium opacity-90 align-middle leading-snug">{pos.allergenName}</td>
                      <td className="py-1.5 px-3 text-center font-mono border-r border-slate-300 align-middle leading-snug">{pos.code}</td>
                      <td className="py-1.5 px-3 text-center align-middle leading-snug">
                        {pos.isTIgE ? (
                          <span className={`text-[12px] font-bold ${pos.isPositive ? 'text-red-700' : 'text-sky-900'}`}>
                            {pos.result || '---'} <span className="text-slate-500 text-[10px] font-normal">(IU/ml)</span>
                          </span>
                        ) : (
                          <div className="flex items-center justify-center">
                            <img 
                              src={getAllergenBadgeSvg(pos.grade, 18)} 
                              width={18} 
                              height={18} 
                              alt={`Độ ${pos.grade}`} 
        ...
        <p className="text-[11.5px] text-slate-500 italic text-right mb-1.5">
          (Chi tiết vui lòng xem trang sau)
        </p>

        {/* Phần MỘT SỐ LƯU Ý (2 cột song song) */}
        <div className="border-t-2 border-slate-300 pt-1.5">
          <h3 className="text-[13px] font-bold text-slate-900 uppercase underline mb-1">
            MỘT SỐ LƯU Ý:
          </h3>
          ...
          <div className="grid grid-cols-12 gap-2.5">
            ...
            <div className="col-span-7 border border-slate-300 rounded text-[11.5px] leading-relaxed bg-slate-50/50 flex flex-col justify-between">
              <div className="text-center font-bold text-red-700 text-[12px] uppercase py-1 px-2 border-b border-slate-300">
                MỘT SỐ TRIỆU CHỨNG THƯỜNG GẶP KHI DỊ ỨNG
              </div>
              <div className="flex-1 flex flex-col justify-center px-3 py-1.5 space-y-1">
              ...
              <p className="text-red-700 font-bold italic px-3 py-1 border-t border-slate-300 text-[11px]">
          ...
          <div className="mt-1.5 border border-sky-300 rounded bg-sky-50/40">
            <table className="w-full text-[11.5px] border-collapse">
              <thead className="bg-sky-100/70 font-bold border-b border-sky-300">
                <tr>
                  <th colSpan={2} className="py-1 px-2 text-center text-sky-900 text-[12px] uppercase tracking-wide align-middle leading-snug">
                    Ghi chú: Tổng nồng độ IgE (TIgE)
                  </th>
                </tr>
                <tr className="border-t border-sky-200">
                  <th className="py-0.5 px-2 text-center border-r border-sky-300 w-1/2 align-middle leading-snug">GIÁ TRỊ BÌNH THƯỜNG (IU/ml)</th>
                  <th className="py-0.5 px-2 text-center w-1/2 align-middle leading-snug">DIỄN GIẢI</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td className="py-1 px-2 text-center font-mono font-bold text-red-600 border-r border-sky-300 text-[12.5px] align-middle leading-snug">&lt;15,0</td>
                  <td className="py-1 px-2 text-center font-semibold text-slate-700 text-[12px] align-middle leading-snug">Mức bình thường — Không tính Độ (+), chỉ có Kết Quả (IU/ml)</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      <div className="text-center text-[10px] text-slate-400 font-mono pt-2 border-t border-slate-200">
        GOLAB CLINICAL LABORATORY • PHIẾU ĐỊNH LƯỢNG IgE ĐẶC HIỆU DỊ NGUYÊN • TRANG {pageNumber ?? 2}
      </div>
    </div>
  );
}
