...
describe('DynamicReportView - QR Fallback & Allergen Pagination Tests', () => {
  afterEach(() => {
    cleanup();
    vi.restoreAllMocks();
  });
  ...
  it('automatically renders an <img> QR code when qrCodeDataUrl is not provided', async () => {
    const template = PRESET_TEMPLATES[0];
    const { container } = render(
      <DynamicReportView
        template={template}
  ...
  it('renders provided qrCodeDataUrl directly without delay', () => {
  ...
  it('chunks 43 allergen items across multiple pages with continuation headers and separate scale page', () => {
  ...
  it('fits <= 14 allergen items on a single detail page without redundant continuation pages', () => {
    const template = PRESET_TEMPLATES[0];
    // 1 TIgE + 10 allergens
